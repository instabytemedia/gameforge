...
**Solution:** A community-driven platform with AI-powered tools for game creation, sharing, and playtesting
...
export function Logo({ size = "default" }: { size?: "sm" | "default" | "lg" }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`font-bold tracking-tight ${s.text}`}>GameForge</span>
    </div>
  );
}
```
